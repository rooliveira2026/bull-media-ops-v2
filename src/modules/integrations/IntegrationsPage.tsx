  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pending: sources.filter(
        (source) =>
          source.status === "pending_credentials" ||
  ...
  return (
    <section>
      ...
      <div className="kpi-grid kpi-grid--compact">
        <KpiCard
          label="Fontes preparadas"
          value={String(stats.prepared)}
          icon={Plug}
          tone="primary"
        />
        <KpiCard
          label="Em validação"
          value={String(stats.validating)}
          icon={Clock3}
          tone="warning"
        />
        <KpiCard
      ...
      <div className="section-card">
        <div className="section-card__header">
          ...
          <p>
            As APIs oficiais ficam preparadas para sprints futuras; a V1 entra
            apenas como exportação controlada.
          </p>
        </div>
            ...
            <tbody>
              {sources.length === 0 ? (
                <tr>
                  <td colSpan={6}>
                    ...
                    <small>
                      Execute a primeira importação controlada da V1 ou mantenha
                      o ambiente em modo mock para preview.
                    </small>
                  </td>
                </tr>
              ) : null}
              ...
              {sources.map((source) => (
                <tr key={source.id}>
                  ...
                  <td>
                    <span className="badge">{statusLabels[source.status]}</span>
                  </td>
                  ...
                  <td>
                    <small>
                      {String(
                        source.metadata.nextStep ??
                          "Preparar credenciais e validação de dados."
                      )}
                    </small>
                  </td>
                </tr>
      ...
      <div className="integration-grid integration-grid--split">
        <div className="section-card">
          ...
          <ul className="simple-list">
            {batches.length === 0 ? (
              <li>
                <strong>Nenhum batch ainda</strong>
                <span>Fonte preparada, aguardando primeira importação.</span>
              </li>
            ) : null}
            ...
            {batches.map((batch) => (
              <li key={batch.id}>
                <strong>
                  {batch.sourceType} ·{" "}
                  {statusLabels[
                    batch.status === "failed" ? "attention" : "connected"
                  ]}
                </strong>
                <span>
                  {batch.recordsImported} importados · {batch.recordsSkipped}{" "}
                  agrupados ou ignorados · checksum{" "}
                  {batch.checksum ?? "a definir"}
                </span>
              </li>
            ))}
          </ul>
          ...
          <ul className="simple-list">
            {logs.length === 0 ? (
              <li>
                ...
                <span>As fontes estão preparadas para a primeira validação.</span>
              </li>
            ) : null}
        ...
        <div className="section-card">
          ...
          <ul className="simple-list">
            <li>
              <strong>
                <Database size={15} /> Supabase staging
              </strong>
              <span>Persistência de fontes, batches e logs de qualidade.</span>
            </li>
            <li>
              <strong>
                <CheckCircle2 size={15} /> Ponte V1
              </strong>
              <span>JSON exportado, normalizado e agrupado antes de persistir.</span>
            </li>
            <li>
              <strong>
                <Plug size={15} /> APIs oficiais
              </strong>
              <span>Preparadas para conexão futura sem reescrever frontend.</span>
            </li>
          </ul>
        </div>
      </div>
